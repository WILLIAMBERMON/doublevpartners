...
class Template {
     ...
     * @param String $value
     */
    public function add_jsw($value) {
        $src = base_url() . 'assets/' . $value;
        $newjs = sprintf('<script type="text/javascript" src="%s.js"></script>', $src);
        if($this->_jsw){
            $this->add_open_graph .= $newjs;
        }else{
            $this->_jsw = $newjs;
        }
        $this->set('_jsw', $this->_jsw);
    }
